...
@Controller // Route : 사용자의 요청(endpoint)와 그데 대한 처리
...
public class EmpController {
	...
	// 전체조회 : Get
	@GetMapping("empList")
	public String emlList(Model model) { // Model = Request + Response
		
		// 1) 기능 수행 = Service
		List<EmpVO> list = empService.emplist();
		
		// 2) 클라이언트에 전달 할 데이터 담기
		model.addAttribute("emps", list); // ← 중요★
	...
	// 단건조회 : Get => QueryString(커맨트 객체 or @RequestParam)
	@GetMapping("empInfo") // empInfo?employeeId(key)=value
	...
	@GetMapping("empUpdate") // empUpdatr?employeeId=value
